...
export const MAX_TAVILY_QUERIES = Math.max(1, Math.min(6, parseInt(process.env.MAX_TAVILY_QUERIES || "5", 10)));
export const TAVILY_MAX_RESULTS = Math.max(1, Math.min(20, parseInt(process.env.TAVILY_MAX_RESULTS || "20", 10)));
...
export const MAX_TOPUP_ATTEMPTS = Math.max(0, Math.min(3, parseInt(process.env.MAX_TOPUP_ATTEMPTS || "1", 10)));
