...
export default function App() {
  ...
  if (!isAuthReady) return <div className="h-screen flex items-center justify-center bg-background"><Loader2 className="animate-spin text-royal-gold" /></div>;

  if (!user) return (
    <div className="h-screen flex flex-col items-center justify-center bg-background p-8 text-center relative overflow-hidden">
      <div className="fixed inset-0 premium-grain" />
      <div className="w-24 h-24 rounded-royal-3xl bg-royal-gold text-white flex items-center justify-center mb-10 shadow-royal-glow relative z-10">
        <FlaskConical className="w-12 h-12" />
      </div>
      <h1 className="text-6xl font-serif font-medium mb-6 relative z-10 tracking-tight">ChemScan</h1>
      <p className="text-royal-muted-foreground mb-12 max-w-sm relative z-10 font-serif italic text-lg opacity-80">Elevate your academic strategy with AI-driven intelligence.</p>
      <Button onClick={() => signInWithGoogle()} className="h-16 px-12 bg-royal-gold hover:bg-royal-gold/90 text-white rounded-royal-xl relative z-10 shadow-2xl font-bold uppercase tracking-widest text-sm transition-all hover:scale-105 active:scale-95">
  ...
  return (
    <div className="min-h-screen bg-background text-royal-obsidian font-sans p-4 md:p-12 relative overflow-x-hidden transition-colors duration-700">
        ...
        <header className="mb-24 text-center relative">
          <div className="absolute top-0 right-0 flex items-center gap-5">
            <div className="hidden md:flex flex-col items-end mr-2">
              <span className="calligraphy text-xl leading-none text-royal-gold lowercase">royal scholar</span>
              <span className="text-[8px] font-black uppercase tracking-[0.3em] text-royal-bronze opacity-40">premium access</span>
            </div>

            <button 
              onClick={() => setShowBuyModal(true)} 
              className="glass-gold px-5 py-2.5 rounded-royal-lg shadow-sm flex items-center gap-2.5 hover:bg-royal-gold/20 transition-all group"
            >
              <Gem className="w-4 h-4 text-royal-gold group-hover:scale-110 transition-transform" />
              <span className="text-[10px] font-black tracking-widest text-royal-gold uppercase">{credits} UNITS</span>
            </button>
            
            <button 
              onClick={toggleTheme}
              className="p-3 rounded-full border border-royal-gold/20 glass-gold hover:border-royal-gold/50 transition-all text-royal-gold shadow-sm"
              title={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}
            >
              {theme === "light" ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}
            </button>

            <button onClick={logOut} className="text-royal-bronze hover:text-royal-gold transition-colors p-2.5 hover:rotate-12 transition-transform"><LogOut className="w-5 h-5" /></button>
          </div>
          ...
          <nav className="flex justify-center gap-4 mt-12 bg-card/40 p-1.5 rounded-full inline-flex border border-royal-border-gold">
            {["generator", "library", "classrooms", "map"].map(t => (
              <button
                key={t}
                onClick={() => setActiveTab(t as any)}
                className={`px-8 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${activeTab === t ? "bg-primary text-primary-foreground shadow-md" : "text-royal-bronze hover:text-royal-obsidian"}`}
              >
                {t}
              </button>
            ))}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {group.items.map(s => (
                              <button
                                ...
                                className={`px-3.5 py-1.5 rounded-full text-[10px] font-bold border transition-all duration-300 ${subject === s ? "bg-primary border-primary text-primary-foreground shadow-md font-serif" : "bg-card/40 border-royal-border-gold text-royal-bronze hover:bg-royal-cream hover:border-royal-bronze/20"}`}
                              >
                                {s}
                              </button>
                            ))}
